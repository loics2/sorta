...
def main():
    ...
    config.readfp(open(os.path.join(args.drop_folder, '.sortaconfig')))
...
def parse_args():
    """parse the arguments'"""
    parser = argparse.ArgumentParser(description="sorta is a program moving your files in the right folders. sorta chooses the destination of the file based on the prefix first, and then on the extension.")
    parser.add_argument('-d', '--drop-folder', dest='drop_folder', type=str, help='path of the folder containing the files to sort', default='~/sorta/')
    ...
    addcmd_parser.add_argument('destination', type=str, help='destination of the moved file containing the prefix/extension')

    rmcmd_parser = subparsers.add_parser('rm', help="remove a prefix or extension")
    rmcmd_parser.add_argument('element', choices=['prefix', 'ext'], help='type of element to remove from the configuration')
